...
std::pair<graph, std::vector<__info>> centroid_decomposition(const graph &G){
  ...
  auto find_centroid = [&](auto &&find_centroid, int v, int p, const int N, const int8_t rank)->std::pair<int, int>{
    int sz = 1;
    for(int c: G[v]){
      if(c == p || info[c].depth < rank) continue;
...
std::vector<long long> frequency_table_of_tree_distance(const graph &G){
  ...
  auto make_deplist = [&](auto &&make_deplist, int v, int p, int d, std::vector<long long> &deplist, const std::vector<__info> &i, const int root_depth)->void{
    ...
    deplist[d]++;
    for(int c: G[v]){
      if(c == p || i[c].depth <= root_depth) continue;
  ...
  auto convo = [&](std::vector<long long> &a, std::vector<long long> &b)->void{
    ...
    if(asum * bsum >= lim) a = internal::convolution_ll(a, b);
    else a = internal::_convolution<lim>(a, b);
  };
  std::vector<long long> ans(n, 0);
  for(int i = 0; i < n; i++){
    if(info[i].size == 1) continue;// leaf
    std::vector<std::vector<long long>> A{{1}};
    std::vector<long long> B{1};
    for(int c: G[i]){
      if(info[c].depth <= info[i].depth) continue;
      A.push_back({0});
    ...
    }else if(A.size() == 4){// xy + yz + zx ??????
      std::vector<long long> x = A[1], &y = A[2], &z = A[3];
      for(int j = 0; j < x.size(); j++) ans[j] += x[j] << 1;
      for(int j = 0; j < y.size(); j++) ans[j] += y[j] << 1;
      for(int j = 0; j < z.size(); j++) ans[j] += z[j] << 1;
      convo(x, y);// xy
      convo(y, z);// yz
      convo(z, A[1]);// zx
      for(int j = 0; j < std::min(n, (int)x.size()); j++) ans[j] += x[j] << 1;
      for(int j = 0; j < std::min(n, (int)y.size()); j++) ans[j] += y[j] << 1;
      for(int j = 0; j < std::min(n, (int)z.size()); j++) ans[j] += z[j] << 1;
    }else{
      convo(B, B);
