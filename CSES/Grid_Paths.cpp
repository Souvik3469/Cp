...
bool e(ll r, ll c, vector<vector<ll>> &vis)
{
    return (r >= 0 and r < 7 and c >= 0 and c < 7 and !vis[r][c]);
}
void dfs(ll r, ll c, ll idx, string &s, vector<vector<ll>> &vis, ll &ans)
{
    if (r == 6 and c == 0)
    {
        if (idx == 48)
    ...
    if (s[idx] == '?' or s[idx] == 'L')
    {
        if (c - 1 >= 0 and !vis[r][c - 1])
        {
            if (!(!e(r, c - 2, vis) and e(r + 1, c - 1, vis) and e(r - 1, c - 1, vis)))
                dfs(r, c - 1, idx + 1, s, vis, ans);
        }
    }
    if (s[idx] == '?' or s[idx] == 'R')
    {
        if (c + 1 < 7 and !vis[r][c + 1])
        {
            if (!(!e(r, c + 2, vis) and e(r + 1, c + 1, vis) and e(r - 1, c + 1, vis)))
                dfs(r, c + 1, idx + 1, s, vis, ans);
        }
    ...
    if (s[idx] == '?' or s[idx] == 'U')
    {
        if (r - 1 >= 0 and !vis[r - 1][c])
        {
            if (!(!e(r - 2, c, vis) and e(r - 1, c + 1, vis) and e(r - 1, c - 1, vis)))
                dfs(r - 1, c, idx + 1, s, vis, ans);
        }
    }
    if (s[idx] == '?' or s[idx] == 'D')
    {
        if (r + 1 < 7 and !vis[r + 1][c])
        {
            if (!(!e(r + 2, c, vis) and e(r + 1, c + 1, vis) and e(r + 1, c - 1, vis)))
                dfs(r + 1, c, idx + 1, s, vis, ans);
        }
...
void solve()
{
    ...
    cin >> s;
    ll ans = 0;
    vector<vector<ll>> vis(7, vector<ll>(7, 0));
    dfs(0, 0, 0, s, vis, ans);
    cout << ans << endl;
}
/*
