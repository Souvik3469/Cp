...
void solve()
{
    ...
    if (amx1.ss != bmx1.ss && bmx1.ss != cmx1.ss && amx1.ss != cmx1.ss)
    {
        cout << amx1.ff + bmx1.ff + cmx1.ff << endl;
        return;
    }
    /*
    if (amx1.ss == bmx1.ss && bmx1.ss == cmx1.ss)
    {
        // ll mx1=max(max(amx1.ss,bmx1.ss))
        vector<pair<ll, ll>> v;
        v.pb({amx1.ff, 1});
        v.pb({bmx1.ff, 2});
        v.pb({cmx1.ff, 3});
    ...
    for (ll i = 0; i < 3; i++)
    {
        for (ll j = 0; j < 3; j++)
        {
            for (ll k = 0; k < 3; k++)
            {
                if (aa[i].ss != bb[j].ss && aa[i].ss != cc[k].ss && bb[j].ss != cc[k].ss)
                {
                    ll res = aa[i].ff + bb[j].ff + cc[k].ff;
                    mx = max(mx, res);
                }
            }
        }
